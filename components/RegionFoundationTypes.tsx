...
function buildBlocks(r: Region) {
  ...
  const plita = {
    title: `Плитный фундамент ${inPrep(r.prepositional)}`,
    tag: 'Основная технология',
    price: `от ${price} ₽/м²`,
    body:
      `Монолитная плита — самый надёжный вариант под дом из газобетона ${inPrep(r.prepositional)}. ` +
      `На ${adj} грунтах ${r.shortName} плита работает как единая платформа и не боится сезонного пучения и подвижек. ` +
      `Цена плитного фундамента под ключ ${inPrep(r.prepositional)} — от ${price} ₽/м², точную стоимость считаем бесплатно после замера.`,
  };

  const lenta = {
    title: `Ленточный фундамент ${inPrep(r.prepositional)}`,
    ...
    body: isWeak
      ? `На ${adj} грунтах ${r.shortName} ленточный фундамент мы обычно не рекомендуем — надёжнее плита с заменой грунта или свайный фундамент. ` +
        `Если участок с плотным основанием — считаем стоимость ленточного фундамента ${inPrep(r.prepositional)} по проекту после геологии.`
      : isSand
        ? `На плотных песчаных грунтах ${r.shortName} ленточный фундамент — рабочий и более дешёвый вариант под лёгкий дом, баню или каркасник. ` +
          `Заливаем мелкозаглублённую и заглублённую ленту из бетона М300. Цену ленточного фундамента ${inPrep(r.prepositional)} рассчитываем по проекту.`
        : `Ленточный фундамент ${inPrep(r.prepositional)} применяем с заглублением ниже глубины промерзания — на ${adj} грунтах мелкозаглублённая лента рискованна. ` +
          `Подходит под лёгкие постройки. Стоимость ленточного фундамента считаем бесплатно после замера.`,
  };

  const svai = {
    title: `Свайный фундамент ${inPrep(r.prepositional)}`,
    ...
    body: isWeak
      ? `На ${adj} грунтах ${r.shortName} свайный фундамент — оптимальное решение: буронабивные или винтовые сваи с ростверком переносят вес дома на плотные слои без дорогой выторфовки. ` +
        `Цену свайного фундамента ${inPrep(r.prepositional)} рассчитываем под ваш дом бесплатно.`
      : `Свайный фундамент ${inPrep(r.prepositional)} берём под каркасные дома, бани и участки со склоном. ` +
  ...
  return (
    <section className="container-x pb-16 max-w-4xl">
      <h2 className="text-2xl md:text-3xl font-extrabold mb-3">
        Какой фундамент выбрать {inPrep(region.prepositional)}: плита, лента или сваи
      </h2>
      <p className="text-brand-mute mb-8 leading-relaxed">
        Тип фундамента {inPrep(region.prepositional)} подбираем под грунт вашего участка — это решает и надёжность,
