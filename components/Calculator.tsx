...
// Держим в одном месте, чтобы калькулятор и таблица не показывали разные суммы.
import {
  SIZE_AREA,
  GROUND_LABEL,
  MATERIAL_LOAD,
  STOREY_LOAD_MULTIPLIER,
  fmtRub as fmt,
} from '@/lib/pricing';
import type { SizeKey, Storeys, Ground, Material } from '@/lib/pricing';
...
export default function Calculator({
  ...
  const [messenger, setMessenger] = useState<'call' | 'telegram' | 'max'>('call');
  ...
  const MESSENGER_KEYS: ReadonlyArray<keyof typeof MESSENGERS> = SITE.max
    ? (['call', 'telegram', 'max'] as const)
    : (['call', 'telegram'] as const);
        ...
        <div className="bg-brand-ink text-white rounded-3xl p-6 md:p-8 shadow-xl sticky top-24">
          ...
          <form onSubmit={submit} className="pt-1">
            ...
            <div className="mb-3">
              ...
              <div className={`grid gap-2 ${SITE.max ? 'grid-cols-3' : 'grid-cols-2'}`}>
